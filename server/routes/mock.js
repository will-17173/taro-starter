...
const mocks = read(`${__dirname}/../../mock`, file => path.extname(file) === '.js').reduce(
  (mock, file) => ({
    ...mock,
    ...require(file)
  }),
...
Object.keys(mocks).forEach(route => {
  ...
  if (validMethods.indexOf(method) !== -1 && url) {
    router[method.toLocaleLowerCase()](url, (req, res, next) => delay(500).then(() => mocks[route](req, res, next)))
  } else {
